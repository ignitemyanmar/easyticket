...
public class SalesbyAgentActivity extends BaseSherlockActivity {
	public static Button from_date, to_date, search;
	private Spinner city_from, city_to, sp_time;
	private String selectedAgentId = "", selectedOperatorId = "",
			selectedCityfromId = "", selectedCitytoId = "",
			selectedTimeid = "", selectedToDate = "", selectedFromDate = "";
	private CitiesbyAgent city_by_agent;
	private List<TimesbyOperator> time_by_agent;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		agent = (Spinner) findViewById(R.id.spn_Brand);
		...
		if (myDevice.getHeight() > myDevice.getWidth()) {
			// portrait
			lyDetails.setTranslationX(myDevice.getWidth());
			lyDetails.getLayoutParams().width = myDevice.getWidth();
		} else {
			// lanscape
		...
		time_by_agent = new ArrayList<TimesbyOperator>();
		time_by_agent.add(new TimesbyOperator("Select All..."));
		...
		if (skDetector.isConnectingToInternet()) {
			...
			agent.setOnItemSelectedListener(brandClickListener);
		} else {
	...
	private OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (v == search) {
				if (skDetector.isConnectingToInternet()) {
					...
					lvOperator.setAdapter(new OperatorDateListAdapter(
							SalesbyAgentActivity.this, tripsbyOperators));
				}
			}
			if (v == from_date) {
				final SKCalender skCalender = new SKCalender(
						SalesbyAgentActivity.this);
				...
				skCalender.setCallbacks(new Callbacks() {
					...
					public void onChooseDate(String chooseDate) {
						...
						String selectedDate = DateFormat.format("yyyy-MM-dd",
			...
			if(v == btn_all_detail){
				...
				editor.putString("agent_id", selectedAgentId);
				editor.putString("agent_name", selectedAgentName);
	...
	private OnItemSelectedListener timeClickListener = new OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
			...
			selectedTimeid = "";
			if (arg2 != 0) {
				selectedTimeid = time_by_agent.get(arg2).getTime();
			}
		}
	...
	private void getCity() {
		...
		String operatorId = pref.getString("user_id", null);
		NetworkEngine.getInstance().getCitybyOperator(accessToken, operatorId,
				new Callback<CitiesbyAgent>() {
					...
					public void success(CitiesbyAgent arg0, Response arg1) {
						...
						city_from.setAdapter(new CityFromAdapter(
								SalesbyAgentActivity.this, city_by_agent
										.getFrom()));
	...
	private void getTripsbyOperator() {
		...
		Log.i("", "Sale Seat : "+selectedAgentId+"," + selectedCityfromId + ","+selectedCityfromName + "," + selectedCitytoId
				+selectedCitytoName + ","+ "," + selectedTimeid + "," + selectedFromDate + ","
				+ selectedToDate);
		...
		NetworkEngine.getInstance().getTripsbyOperator(accessToken,
				...
				new Callback<List<TripsbyOperator>>() {
					...
					public void success(List<TripsbyOperator> arg0,
							Response arg1) {
						// TODO Auto-generated method stub
	...
	private void getBrandData() {
		...
		NetworkEngine.getInstance().getAgentbyOperator(accessToken, user_id,
				new Callback<AgentsbyOperator>() {
					...
					public void success(AgentsbyOperator arg0, Response arg1) {
						...
						agent.setAdapter(new AgentAdapter(
								SalesbyAgentActivity.this, agent_by_operator
										.getAgents()));
					}
	...
	private void getTimeData() {
		...
		String accessToken = pref.getString("access_token", null);
		String operatorId = pref.getString("user_id", null);
		NetworkEngine.getInstance().getTimebyOperator(accessToken, operatorId,
				new Callback<List<TimesbyOperator>>() {

					public void success(List<TimesbyOperator> arg0,
							Response arg1) {
						// TODO Auto-generated method stub
						time_by_agent.addAll(arg0);
						sp_time.setAdapter(new TimeAdapter(
								SalesbyAgentActivity.this, time_by_agent));
					}
