...
public class DetailReportExcelUtility {
  ...
  public DetailReportExcelUtility(List<SaleDetail> saleDetail, String filename) {
  ...
  private void createLabel(WritableSheet sheet)
    ...
    // Write a few headers
    addCaption(sheet, 0, 0, "Vr No");
    addCaption(sheet, 1, 0, "Customer Name");
    addCaption(sheet, 2, 0, "Seat No.");
    addCaption(sheet, 3, 0, "Trip Time");
    addCaption(sheet, 4, 0, "Trip Date");
    addCaption(sheet, 5, 0, "Trip City");
    addCaption(sheet, 6, 0, "Pay-Price");
    addCaption(sheet, 7, 0, "Total Seat");
    addCaption(sheet, 8, 0, "Total Amount");
  ...
  private void createContent(WritableSheet sheet) throws WriteException,
    ...
    for(SaleDetail sale: saleDetail){
    	...
    	addLabel(sheet, 0, i,sale.getId().toString());
    	addLabel(sheet, 1, i,sale.getCustomer().toString());
		String seatNo = "";
		for(Saleitem saleitem:sale.getSaleitems()){
			seatNo += saleitem.getSeatNo()+", ";
		}
		addLabel(sheet, 2, i,seatNo.toString());
		addLabel(sheet, 3, i,sale.getDepartureTime().toString());
		addLabel(sheet, 4, i,sale.getDepartureDate().toString());
		addLabel(sheet, 5, i,sale.getTrip().toString());
		addLabel(sheet, 6, i,sale.getPrice().toString());
		addLabel(sheet, 7, i,sale.getSaleitems().size()+"");
		addLabel(sheet, 8, i,sale.getAmount().toString());
