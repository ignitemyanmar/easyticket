...
public class BusConfirmActivity extends BaseSherlockActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < selectedSeat.length; i++) {
			...
			//chk_free.setTag(i+1 * 200); // For container layout of free remarks.
			chk_free.setTag(i);
			chk_free.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					...
					if(isChecked)
						lst_layout_free_ticket.get(position).setVisibility(View.VISIBLE);
					else
						lst_layout_free_ticket.get(position).setVisibility(View.GONE);
				}
			});
			...
			//chk_discount.setId(i+1 * 300); // For discount check box of discount.
			lst_discount_chk.add(chk_discount);
			//chk_discount.setTag(i+1 * 400);// For edit text of discount amount.
			chk_discount.setTag(i);
			chk_discount.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					// TODO Auto-generated method stub
					Integer position = (Integer) buttonView.getTag();
					if(isChecked)
						lst_discount_edt.get(position).setVisibility(View.VISIBLE);
					else
						lst_discount_edt.get(position).setVisibility(View.GONE);
				}
			});
		...
		for (int i = 0; i < selectedSeat.length; i++) {
			...
			seats.add(
					new ConfirmSeat(BusOccurence,selectedSeat[i].toString(), edt_buyer
							...
							.toString(),lst_ticket_no.get(i).getText().toString(),
							lst_free_chk.get(i).isChecked(), free_ticket_remark,
							discount));
		}
	...
	private void getAgent(){
		...
		NetworkEngine.getInstance().getAllAgent(accessToken,user_id, new Callback<AgentList>() {
			...
			public void success(AgentList arg0, Response arg1) {
				...
				Log.i("", "Agent List: "+ agentList.size());
