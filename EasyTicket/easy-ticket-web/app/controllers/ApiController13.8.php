...
class ApiController extends BaseController
{
	...
	public static function tripautocreateold($operator_id){
		if(!$operator_id){
			return false;
		}
		$today=date('Y-m-d');
		...
		if($objtrips){
			foreach ($objtrips as $trip) {
				if(strtolower($trip->available_day)=='daily'){
					...
					$nextMonth 			=date("m", strtotime($month));
					$days_in_nextMonth  =date("t", strtotime($month));
					$days_in_currentMonth  =date("t", strtotime($cMonth));
	...
	public function getSeatPlan(){
		...
		if($date && !$time){
			...
			$buslist=BusOccurance::whereoperator_id($operator_id)->wherefrom($from)->whereto($to)->where('departure_date','=',$date)->wherestatus(0)->get();
		}
		elseif($date && $time){
			$date=date('Y-m-d', strtotime($date));
			$buslist=BusOccurance::whereoperator_id($operator_id)->wherefrom($from)->whereto($to)->where('departure_date','=',$date)->where('departure_time','=',$time)->wherestatus(0)->get();
		}
		elseif(!$date && !$time)
		{
			$buslist=BusOccurance::whereoperator_id($operator_id)->wherefrom($from)->whereto($to)->wherestatus(0)->get();
		}else{
		}
    ...
    public function postSaleComfirm(){
    	...
    	if($json_tickets){
    		...
    		$objsaleorder->orderdate=$order_date ? $order_date : date('Y-m-d');
    ...
    public function getCreditSale(){
    	...
    	if($order_id){
    		...
    		if($operator_id && $agent_id){
	    		...
	    		$response=SaleOrder::whereoperator_id($operator_id)->whereagent_id(0)->wherecash_credit(2)->with(array('saleitems'))->take($limit)->skip($offset)->get(array('id', 'orderdate', 'agent_id', 'operator_id'));
	    	}elseif(!$operator_id && $agent_id){
    ...
    public function getTripReportByDateRanges(){
		...
		if($operator_id && !$agent_id && !$from && !$to && !$start_date && !$end_date && !$departure_time){
			$busoccurance_ids=BusOccurance::whereoperator_id($operator_id)->lists('id');
			// dd($busoccurance_ids);
		}else{
			if(!$operator_id && !$agent_id){
				$response =array();
					...
					if($occuranceids_bydaily){
						if(!$from_city && !$to_city){
							foreach ($occuranceids_bydaily as $occuranceid) {
								...
								$fromto=Busoccurance::whereid($occuranceid)->first();
								$from_city=City::whereid($fromto->from)->pluck('name');
								$to_city=City::whereid($fromto->to)->pluck('name');
								if($seatplanid){
	...
	public function getTripsReportByDaily(){
		...
		if(!$operator_id || !$date){
			$response['message']='Required fields are operator_id and date.';
			return Response::json($response);
		}
		$busoccurance_id=array();		
		if(!$from && !$to && $date){
			$orderids=SaleOrder::whereorderdate($date)->lists('id');
			if($orderids)
			$busoccurance_id=SaleItem::wherein('order_id',$orderids)->groupBy('busoccurance_id')->lists('busoccurance_id');
		}else{
			if($departure_time){
    ...
    public function getSeatReportByTrip(){
    	...
    	if(!$bus_id){
    		...
    		if($agent_id && $date){
		    	$order_ids=SaleOrder::whereorderdate($date)->whereagent_id($agent_id)->lists('id');
	    	}elseif(!$agent_id && $date){
		    	$order_ids=SaleOrder::whereorderdate($date)->lists('id');
	    	}elseif($agent_id && !$date){
	    		...
	    		if($agent_id && !$operator_id){
			    	...
			    	$order_ids=SaleOrder::whereoperator_id($operator_id)->lists('id');
	    		}
	    	}
	    	$orderids=SaleItem::wherein('order_id',$order_ids)->groupBy('order_id')->lists('order_id');
    	}
    	...
    	if($orderids){
    		if($invoice_no)
    			$saletickets =SaleItem::whereorder_id($invoice_no)->get();
    		else
    			$saletickets =SaleItem::wherein('order_id', $orderids)->wherebusoccurance_id($bus_id)->get();
    		
    		if($saletickets){
    			if($bus_id)
    				$objbusoccurance =BusOccurance::whereid($bus_id)->first();
    			else{
    				$busoccurance_id=SaleItem::whereorder_id($invoice_no)->pluck('busoccurance_id');
    				$objbusoccurance =BusOccurance::whereid($busoccurance_id)->first();
    			}
    			$fromto=Busoccurance::whereid($bus_id)->first();
    			$from_city 	=City::whereid($fromto->from)->pluck('name');
    			$to_city 	=City::whereid($fromto->to)->pluck('name');
    ...
    public function getSaleReportbyAgentGroupold(){
    	$agentgroup_id=$branch_agent_id	= Input::get('agent_id');
    	$agentgroup_id		= Agent::whereid($branch_agent_id)->pluck('agentgroup_id');
...
*/
    ...
    public function getDailyAdvancedTripsold(){
		$report_info 			=array();
    	$date 					=Input::get('date');
		$operator_id  			=Input::get('operator_id');
		$agent_id  				=Input::get('agent_id');
    ...
    public function getDailyReportbydeparturedate(){
    	...
    	$busids=null;
    	if($operator_id && !$departure_date && !$from && !$to){
    		$objbusoccuranceids 	=BusOccurance::whereoperator_id($operator_id)->lists('id');
    	}else{
    		if(!$time && !$departure_date && $from && $to){
    ...
    public function postAgentDepositold(){
    	$agent_id 		=Input::get('agent_id');
    	$operator_id 	=Input::get('operator_id');
    ...
    public function getAgentCreditByDateRangeold(){
    		...
    		if($response){
    			...
    			foreach ($response as $row) {
    				if($busids && count($row->saleitems) >0){
    					foreach ($busids as $busid) {
    						if($busid==$row->saleitems[0]->busoccurance_id){
    							$response[$j]=$row;
    						}
    					}
    	...
    	if($filter){
    		...
    		if($objbus){
    			foreach ($filter as $row) {
					...
					$operator=Operator::whereid($row['operator_id'])->pluck('name');
					$objagent=Agent::whereid($row['agent_id'])->first();
    		...
    		if($response){
	    		...
	    		foreach ($response as $row) {
	    			...
	    			$response[$i]['agent']='-';
	    			if($objagent){
    					$response[$i]['agent']=$objagent->name;
    					$commissiontype=CommissionType::whereid($objagent->commission_id)->pluck('name');
    					$response[$i]['commission']=$objagent->commission;	
    					if(strtolower($commissiontype)=='fixed'){
    						$response[$i]['commission_type']='fixed';	
    						$response[$i]['agent_commission']=$objagent->commission * $tickets ;	
    					}else{
    						$response[$i]['commission_type']='percentage';	
    						$response[$i]['agent_commission']=($amount * $objagent->commission) / 100;	
    					}
    				}
	    			$response[$i]['trip']=$trip;
	    			$response[$i]['total_ticket']=$tickets;
	    			$response[$i]['price']=$price;
	    			$response[$i]['amount']=$amount;
	    			// $response[$i]['agent_commission']=$response[$i]['commission'];
	    			$response[$i]['grand_total']=($amount - $response[$i]['agent_commission']);
	    			$i++;
	    		}
	    	}
	    	return Response::json($response);
    ...
    public function getAgentListByoperatorold(){
    	$operator_id=Input::get('operator_id');
    	if(!$operator_id){
    		$response['status']=0;
    		$response['message']="Request parameter is required.";
    ...
    public function postAgentCreditByDateRangePaymentold(){
    	$start_date=Input::get('start_date');
    	$end_date=Input::get('end_date');
    	$from=Input::get('from');
    	$to=Input::get('to');
    	...
    	if($response){
    		...
    		$total_amount=SaleItem::wherein('order_id', $response)->sum('price');
    	}
    	...
    	if($total_amount >0){
    		...
    		if($current_balance < $total_amount){
    			$debit = $total_amount - $current_balance;
    		}
    		$objdepositpayment_trans->deposit=$current_balance;
    		$objdepositpayment_trans->balance=$balance;
    		$objdepositpayment_trans->debit=$debit;
    ...
    public function postAgentCreditByDateRangePayment5_8(){
    	$start_date=Input::get('start_date');
    	$end_date=Input::get('end_date');
    	$from=Input::get('from');
    	$to=Input::get('to');
    ...
    public function getSaleList(){
    	...
    	{
    		if($operator_id){
    			$busoccuranceid=Busoccurance::whereoperator_id($operator_id)->wherefrom($from)->whereto($to)->wheredeparture_date($departure_date)->wheredeparture_time($departure_time)->lists('id');
    		}else{
    			$busoccuranceid=Busoccurance::wherefrom($from)->whereto($to)->wheredeparture_date($departure_date)->wheredeparture_time($departure_time)->lists('id');
    		}
    	}
    	else
    	{
    ...
    public function postAgentCommissionByTrip(){
    	...
    	$check_exiting=AgentCommission::wheretrip_id($trip_id)->whereagent_id($agent_id)->first();
