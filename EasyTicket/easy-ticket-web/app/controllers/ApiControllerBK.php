...
class ApiController extends BaseController
{
				...
				if($objcloseseatinfo){
					...
					foreach ($objseatinfo as $rows) {
						...
						$checkoccupied_seat =SaleItem::wherebusoccurance_id($busoccurance_id)
												->whereseat_no($rows->seat_no)->first(array('order_id','ticket_no','name','phone','nrc_no','agent_id','discount'));
	...
	public function getTimeLists(){
    	 ...
    	 */
			$expired_ids=SaleOrder::wherebooking(1)->where('departure_datetime','<',$this->today)->lists('id');
	    	if($expired_ids){
	    		foreach ($expired_ids as $expired_id) {
