	...
	public function getcart($id){
		...
		if($objsaleitems){
			foreach ($objsaleitems as $ticket) {
				...
				$temp['operator_id']	=$ticket['operator_id'];
				$temp['operator']		=Operator::whereid($ticket['operator_id'])->pluck('name');
		...
		return View::make('bus.cartview', array('response'=> $tickets));
	}
		...
		$ticket_no		=Input::get('ticket_no');
    		...
    		foreach ($seat_no as $seatno) {
    			...
    			if($objsaleitems){
	    			...
	    			$objsaleitems->ticket_no		=$ticket_no;
	    			$objsaleitems->update();
    			}
	    		$i++;
    		}
