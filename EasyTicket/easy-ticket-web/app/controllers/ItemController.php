...
class ItemController extends \BaseController {
	...
	public function index()
	{
      ...
      $items= Item::whereuser_id($user_id)->orderBy('id', 'DESC')->get();
      if(count($items)>0){
        $i=0;
  ...
  {       
        $user_id=Auth::user()->id;
        $permit_count=Auth::user()->permit_count; 
        $counts=Item::whereuser_id($user_id)->count();
        if($permit_count==0){
          $message="Sorry!. You don't have permit to upload items.";
          return Redirect::to('item')->with('message',$message);
        }elseif($permit_count!=0 && $permit_count== $counts){
          $message="You have been uploaded items that permit item count.";
          return Redirect::to('item')->with('message',$message);
        }else{
        ...
        // if(count($shops)> 0){
            ...
            $objitem->service        =Input::get('service') ? Input::get('service') : "";
            $objitem->qty_per_control=Input::get('qty_per_control') ? Input::get('qty_per_control') : 0;
            $objitem->save();
            ...
            if(count($matchcolorsize)>0){
              ...
              foreach ($matchcolorsize as $itemdt) {
                ...
                $prefix_menu=Menu::whereid(Input::get('menu'))->pluck('itemcode_prefix');
                ...
                $itemcode=$prefix_menu.$prefix_category; 
            ...
            if(count($quantityrange) >0){
              ...
              foreach ($unique_qtyrange as $qtyrange) {
                if($pricebyqty[$c] !='' && $pricebyqty[$c] > 0){
  ...
  public function postupdate($id){
    ...
    $image=Input::get('image') ? Input::get('image') : "";
    $backgroundcolor_code=Input::get('backgroundcolor_code') ? Input::get('backgroundcolor_code') : '';
    if($image)
      $objitem->home_image=$image;
      $objitem->backgroundcolor_code=$backgroundcolor_code;
    $objitem->update();
    return Redirect::to('/');
  }
  /**
  ...
  {
    $responsecombo=array();
    $objitem= Item::with(array('itemdetail','itemimages','pricebyqtyranges','compareprice','shop','menu','category', 'subcategory','itemcategory'))->find($id);
    $shop=Shop::whereuser_id(Auth::user()->id)->get(array('id','name','name_mm'));
