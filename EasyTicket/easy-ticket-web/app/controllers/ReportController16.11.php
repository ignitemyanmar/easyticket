...
class ReportController extends BaseController
{
	...
	public function getTripslistreportOperatormod()
	{
		$report_info 			=array();
		...
		$order__ids=SaleOrder::where('orderdate','>=',$start_date)->where('orderdate','<=',$end_date)->lists('id');
		$order_busids=SaleItem::wherein('order_id',$order__ids)->wherein('busoccurance_id',$busoccurance_ids)->groupBy('busoccurance_id')->lists('busoccurance_id');
		
		$busoccurance_ids=array_intersect($busoccurance_ids, $order_busids);
		// dd($busoccurance_ids);
		...
		if($busoccurance_ids){
			...
			if(count($orderdates) >0){
				...
				foreach ($orderdates as $order_date) {
					if($agent_id){
						$orderids_bydaily		=SaleOrder::whereorderdate($order_date)->whereagent_id($agent_id)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
					}else{
						$orderids_bydaily		=SaleOrder::whereorderdate($order_date)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
					}
					...
					if($occuranceids_bydaily){
						...
						$temp['total_amout']			=SaleItem::wherein('order_id', $orderids_bydaily)->sum('price');
	...
	public function getTripsSellingReportbyDaily($date){
		...
		if($busoccurance_id){
			...
			if($orderidsbyoccuranceid && !$agent_id){
				$filterorderidsbydate  	=SaleOrder::wherein('id', $orderidsbyoccuranceid)->whereorderdate($date)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
				...
				$filterorderidsbydate  	=SaleOrder::wherein('id', $orderidsbyoccuranceid)->whereagent_id($agent_id)->whereorderdate($date)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
			}else{
	...
	public function getTripsSellingReportbyBusid($bus_id){
    	...
    	$orderids=array();
    	if($agent_id){
	    	$orederids=SaleOrder::whereorderdate($date)->whereagent_id($agent_id)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
    	}else{
	    	$orederids=SaleOrder::whereorderdate($date)->where('name','!=','')->orwhere('booking','=',1)->lists('id');
    	}
    	$seat_info=array();
	  ...
	  public function getDailyReportByDepartureDate(){
		...
		public function getDailyReportByDepartureDatesearch(){
	    	...
	    	if(!$time && $from && $to && $departure_date){
	    		...
	    		$objbusoccuranceids 	=BusOccurance::whereoperator_id($operator_id)->wherefrom($from)->whereto($to)->wheredeparture_time($time)->lists('id');
	    	}
