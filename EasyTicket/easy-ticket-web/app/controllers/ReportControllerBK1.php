	    		...
	    		foreach ($objagentids as $row) {
	    			...
	    			if($check_existed == -1){
    					...
    					if($agent_commission){
    						...
    						$temp['total_amount'] = (($price * $temp['sold_tickets']) - ($agent_commission->commission * $temp['sold_tickets'])) - $temp['discount'];
    					}
	    				else{
	    					$temp['commission']   = $objtrip->commission;
	    					$temp['total_amount'] = (($price * $temp['sold_tickets']) - ($objtrip->commission * $temp['sold_tickets'])) - $temp['discount'];
	    				}
	    				if($checkowner==1){
    					...
    					if($agent_commission){
    						$temp['commission']   = $agent_commission->commission;
    						$total_amount = (($sold_tickets * $price) - ($agent_commission->commission * $sold_tickets)) - $discount;
    					}
	    				else{
	    					$temp['commission']   = $objtrip->commission;
	    					$total_amount = (($sold_tickets * $price) - ($objtrip->commission * $sold_tickets)) - $discount;
	    				}
	    				...
	    				$checkowner=Agent::whereid($row->agentid)->pluck('owner');
	    				$response[$check_existed]['owner']			=$checkowner;
