                  ...
                  <div class="responsive span12" data-tablet="span12" data-desktop="span12">
                     <div class="portlet box light-grey">
                        ...
                        <div class="row-fluid search-default">
                           ...
                           <div class="span10">
                                 <form action="/report/operator/trip/daterangesbycode" method="get" class="horizontal-form">
                                                   ...
                                                   <select name="agent_id" class="m-wrap span12 chosen">
                                                      ...
                                                      @if($search['agent'])
                                                         @foreach($search['agent'] as $row)
                                                            <option value="{{$row['code_no']}}" @if($row['code_no']==$search['agent_id']) selected @endif>{{$row['name']}}</option>
                                                         @endforeach
                                                      @endif
                                                   </select>
                                                </div>
                           ...
                           <table class="table table-striped table-bordered table-advance table-hover" id="sample_editable_1">
                              <thead>
                                 <tr>
                                    ...
                                    <th style="width:0;"><a class="btn small green blue-stripe imagechange" id="" href="/triplist/{{$search['start_date'].','.$search['end_date']}}/agentdetailreportbycode?access_token={{Auth::user()->access_token}}&f={{$search['from']}}&t={{$search['to']}}&agentgroup={{$search['agentgroup_id']}}&a={{$search['agent_id']}}&agentrp={{$search['agent_rp']}}&time={{$search['time']}}">အေသးစိတ္(All)</a></th>
                                 </tr>
                              </thead>
                                 @if($response)
                                    <tbody>
                                          ...
                                          @if(count($rows)>0)
                                             @foreach($rows as $result)
                                                <tr>
                                                   ...
                                                   <td>
                                                      <a class="btn mini green-stripe imagechange" id="" href="/triplist/{{$result['departure_date']}}/agentdetailreportbycode?access_token={{Auth::user()->access_token}}&trip_id={{$result['id']}}&a={{$result['agent_id']}}&agentrp={{$search['agent_rp']}}">အေသးစိတ္ၾကည့္ရန္</a>
                                                   </td>
                                                </tr>
                                                <?php 
      ...
      function loadagentbranches(agentgroup_id){
         ...
         $.get('/agentbranches/'+agentgroup_id+'?access_token='+_token,function(data){
            ...
            for(var i=0; i<data.length; i++){
               result +='<option value="'+data[i].code_no+'">'+data[i].name+'</option>';
            }
            result +='</select>';
