...
<style type="text/css">
	ul.pagination li{border:1px solid #eee;}
...
<div class="row">
	<div class="large-10 medium-12 column nopadding">
		<!-- <div class="row nopadding">
			<div class="tabs large-12 medium-12 nopadding hot_title">
					<ul class="navbar"> 
						<li><a href="#panel2-1">အဝတ္အစား ပစၥည္းမ်ား</a></li> 
						<li><a href="#panel2-2">အလွျပင္ပစၥည္းမ်ား</a></li> 
						<li><a href="#panel2-3">လ်ွပ္စစ္ ပစၥည္မ်ား</a></li> 
						<li><a href="#panel2-4">ကေလးသုံးပစၥည္းမ်ား</a></li> 
						<li><a href="#panel2-5">မီးဖုိေခ်ာင္သုံးပစၥည္းမ်ား</a></li> 
					</ul>
			</div>
		</div> -->
		<div class="row">
			<div class="large-9 columns">
				<h3 class="category_title"><a href="time-sale/morning">{{$title}}</a></h3>
			</div>
			<div class="large-3 columns hide-for-small hide-for-medium nopadding txt_lable right">
				@if(count($timesale)>0)
					@if ($timesale->getLastPage() > 0)
						<?php $previousPage = ($timesale->getCurrentPage() > 1) ? $timesale->getCurrentPage() - 1 : 1; ?>  
						<ul class="pagination"> 
							<li class="item{{ ($timesale->getCurrentPage() == 1) ? ' disabled' : '' }}"> 
								@if($timesale->getCurrentPage()>1)
								    <a href="{{ $timesale->getUrl($previousPage) }}&search={{$search}}">
								    	« Prev
								    </a>
							    @else
								    « Prev
						    ...
						    @for ($i = 1; $i <= $timesale->getLastPage(); $i++)
							  @if($timesale->getCurrentPage() == $i) 
							  <li class="item{{ ($timesale->getCurrentPage() == $i) ? ' active' : '' }}">
								    <a href="{{ $timesale->getUrl($i) }}&search={{$search}}">
								      {{ $i }}
								    </a>
							  </li>
							  @endif
						    ...
						    <li class="item{{ ($timesale->getLastPage() == $i) ? ' active' : '' }}">
							    <a href="{{ $timesale->getUrl($timesale->getLastPage()) }}&search={{$search}}">
							    	{{ $timesale->getLastPage() }}
							    </a>
						    </li>
						  	<li class="item{{ ($timesale->getCurrentPage() == $timesale->getLastPage()) ? ' disabled' : '' }}">
							@if($timesale->getCurrentPage() != $timesale->getLastPage())
								<a href="{{ $timesale->getUrl($timesale->getCurrentPage()+1) }}&search={{$search}}">
							    	Next <span>»</span>
								</a>
							@else
							  	Next <span>»</span>
		...
		@if(count($response['timesale'])> 0)
			<?php $i=1;$j=1; $totalcount=count($response['timesale']);?>
					...
					<div class="large-2-half columns free_get_items left">
						<div class="grid_four_frame">
							<a href="/itemdetail/{{$row->id}}">
								<div class="image_frames light-green">
									<img class="center_photo" src="../../../../itemphoto/php/files/medium/{{$row->image}}">
								</div>
								<div class="item_description">
									<div class="timesale"><div class="countdown" data-seconds="1000000" style="position:absolute;top:180px;z-index:9;"></div></div>
	...
	<div class="large-2 medium-12 column nopadding" style="padding-left:5px; ">
		...
		<div class="row nopadding right_border">
			@if(count($response['hotitems'])>0)
				@foreach($response['hotitems'] as $row)
					<div class="large-12 medium-3 column left nopadding hot_item">
						<div class="hot_item_price">{{$row->price}} က်ပ္</div>
						<div class="preview_photo">
							<a href="../../../../itemdetail/{{$row->id}}"><img class="center_photo" src="../../../../itemphoto/php/files/medium/{{$row->image}}"></a>
						</div>
						<div class="row nopadding">
						...
						<div class="row">
							<div class="column nopadding list_item_title"><a href="../../../../itemdetail/18">Latest Electornic </a></div>
						</div>
					</div>
				@endforeach
			@endif
