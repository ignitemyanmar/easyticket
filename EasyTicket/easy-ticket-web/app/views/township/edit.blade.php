						   ...
						   <div class="span12">
						      ...
						      <div class="portlet box blue">
						         <div class="portlet-title">
						            <h4><i class="icon-reorder"></i>Update Township</h4>
						         </div>
						         <div class="portlet-body form">
						            <!-- BEGIN FORM-->
						            <form class="form-horizontal" action = "/township/update/{{$response->id}}" method= "post" enctype="multipart/form-data">
						               	<div class="control-group">
						                  <label class="control-label">City</label>
						                  <div class="controls">
						                  	<select name="city_id" id="city_id">
						                  		@foreach($city as $row)
						                  			<option value="{{$row->id}}" @if($row->id== $response['city_id']) selected @else @endif>{{$row->name. '( '. $row->name_mm .' )'}}</option>
						                  		@endforeach
						                  	</select>
						                ...
						                <div class="form-actions">
						                  ...
						                  <a href="/township"><button type="button" class="btn">Cancel</button></a>
						                </div>
						            </form>
						            <!-- END FORM-->           
						         </div>
